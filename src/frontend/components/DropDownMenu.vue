<template>
    <div v-click-outside="{ handler: onClickOutsideMenu, capture: true }" class="cursor-default relative inline-block text-left">
        ...
        <Transition
            ...
            <div v-if="isShowingMenu" @click.stop="hideMenu" class="overflow-hidden absolute right-0 z-10 mr-4 w-56 rounded-md bg-white shadow-md border border-gray-200 focus:outline-none dark:bg-zinc-800 dark:border-zinc-700" :class="[ dropdownClass ]">
                <slot name="items"/>
            </div>
        </Transition>
